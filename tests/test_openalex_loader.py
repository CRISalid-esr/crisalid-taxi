...
def test_openalex_hierarchy_display():
    ...
    print("=" * 120)
    domains = loader.formatter.get_domains_embedding(loader.domains)
    print("\n".join(domains[:2]))

    print("\n" + "=" * 120)
    print("📂 FIELDS EMBEDDING TEXT")
    print("=" * 120)
    fields = loader.formatter.get_fields_embedding(loader.fields)
    print("\n".join(fields[:2]))

    print("\n" + "=" * 120)
    print("📁 SUBFIELDS EMBEDDING TEXT")
    print("=" * 120)
    subfields = loader.formatter.get_subfields_embedding(loader.subfields)
    print("\n".join(subfields[:2]))

    print("\n" + "=" * 120)
    print("📄 TOPICS EMBEDDING TEXT")
    print("=" * 120)
    topics = loader.formatter.get_topics_embedding(loader.topics)
    print("\n".join(topics[:2]))
    ...
    print(
        f"📊 Summary: {summary['domains']} domains | {summary['fields']} fields | {summary['subfields']} subfields | {summary['topics']} topics"
    )
    print("=" * 120 + "\n")
